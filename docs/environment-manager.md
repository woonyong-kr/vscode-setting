...
이 저장소의 목표는 Woonyong의 개발 환경을 다른 macOS 또는 Windows 환경에서 최대한 재현할 수 있게 보관하는 것입니다.
기준점은 VS Code이며, PyCharm/IntelliJ/Cursor 같은 다른 IDE가 이 기준을 따라오게 유지합니다.
...
JetBrains keymap 일괄 적용 스크립트는 현재 macOS 기준으로 제공됩니다. 공통 keymap source는 `jetbrains/Codex VSCode.xml` 이고, PyCharm/IntelliJ 같은 JetBrains 계열에 동일하게 적용하는 것을 목표로 합니다.
...
- 컨벤션 변경은 관련 언어 문서와 AI 지침 파일(`AGENTS.md`)까지 함께 갱신합니다.
